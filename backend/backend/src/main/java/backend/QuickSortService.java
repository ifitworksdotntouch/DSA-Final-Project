...
public class QuickSortService {
    ...
    private int hoarePartition(int[] arr, int low, int high, int depth){
        ...
        recordStep(arr, pivotIdx,
            List.of(), List.of(), new ArrayList<>(sortedIndices),
            low, high, depth,
            "Pivot selected: " + pivotValue + " (median-of-three), placed at index " + pivotIdx);
        int left = low - 1;
        int right = high + 1;
        ...
        while (true) {
            ...
            if (left >= right) {
                sortedIndices.add(right);
                recordStep(arr, pivotIdx,
                    List.of(), List.of(), new ArrayList<>(sortedIndices),
                    low, high, depth,
                    "Partitioning complete. Pivot " + pivotValue + " is at final position " + right + ".");
                    return right;
            }
    ...
    private int medianOfThree(int[] arr, int low, int high, int depth) {
        ...
        recordStep(arr, high,
            ...
            low, high, depth,
            "Median " + arr[high] + "moved to pivot position at (index " + high + ")");
        return arr[high];
    }
    ...
    private void recordStep(int[]arr, Integer pivotIndex, List<Integer> compareIndices, List<Integer> swapIndices, List<Integer> sortedIndices, int left, int right, int depth, String message){
        steps.add(new SortStep(
            arr, pivotIndex, compareIndices, swapIndices, sortedIndices, left, right, depth, comparisons, swaps, message
        ));
    }
}
