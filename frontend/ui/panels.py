...
class LeftPanel:
    ...
    def input_section(self):
        ...
        ctk.CTkButton(btn_row, text="Manual", width=108, height=30, font=FONT_SMALL, 
                      fg_color=ACCENT_BLUE, hover_color=ACCENT_BRIGHT, corner_radius=6, 
                      command=self.app.focus_entry).pack(side="left", padx=(0, 6))
        ctk.CTkButton(btn_row, text="Random", width=108, height=30, font=FONT_SMALL, 
                      fg_color=ACCENT_BLUE, hover_color=ACCENT_BRIGHT, corner_radius=6, 
                      command=self.app.random_input).pack(side="left")
        
        self.app.array_entry = ctk.CTkTextbox(input_frame, height=48, fg_color=BG_INPUT, 
                                              text_color=TEXT_PRIMARY, border_color=BORDER_BRIGHT, 
                                              border_width=1, corner_radius=6, font=FONT_MONO, wrap="word")
        self.app.array_entry.pack(fill="x", padx=10, pady=(0, 6))
        
        ctk.CTkButton(input_frame, text="Generate Array", height=32, font=FONT_LABEL, 
                      fg_color=ACCENT_BLUE, hover_color=ACCENT_BRIGHT, corner_radius=6, 
    ...
    def pivot_section(self):
        ...
        ctk.CTkButton(header, text="?", width=22, height=22, font=FONT_SMALL, 
                      fg_color=ACCENT_BLUE, hover_color=ACCENT_BRIGHT, corner_radius=11, 
                      command=self.app.show_pivot_help).grid(row=0, column=1, sticky="e")
        ...
        for value, label in [("first", "First element"),
            ...
            ctk.CTkRadioButton(pivot_frame, text=label, value=value, variable=self.app.pivot_var, 
                               font=FONT_SMALL, text_color=TEXT_PRIMARY, fg_color=ACCENT_BRIGHT, 
    ...
    def speed_section(self):
        ...
        self.app.speed_val_label = ctk.CTkLabel(speed_frame, text="1.0×", font=FONT_LABEL, text_color=ACCENT_BRIGHT)
        self.app.speed_val_label.pack()
        
        slider = ctk.CTkSlider(speed_frame, from_=0.25, to=3.0, variable=self.app.speed_var, 
                               number_of_steps=11, progress_color=ACCENT_BLUE, button_color=ACCENT_BRIGHT, 
    ...
    def status_section(self):
        ...
        self.app.status_label = ctk.CTkLabel(row, textvariable=self.app.status_var, font=FONT_LABEL, text_color=COLOR_PIVOT)
        self.app.status_label.pack(side="right", padx=6)
...
class CenterPanel:
    ...
    def controls(self):
        ...
        buttons = [
            ("▶  Start",  "lime",   self.app.start_sort),
            ("⏸  Pause",  "#ffa502", self.app.pause_sort),
            ("⏭  Step",   "#b44fff", self.app.step_sort),
            ("↺  Reset",  COLOR_PIVOT, self.app.reset_sort),
        ] 
        for text, color, cmd in buttons:
            ctk.CTkButton(inner, text=text, width=110, height=36, font=FONT_CTRL, 
                          text_color=color, fg_color=BG_INPUT, hover_color=BG_PANEL, 
                          border_width=1, border_color=BORDER_BRIGHT, corner_radius=6, 
    ...
    def quote_bar(self):
        ...
        choices = ['"Divide and conquer — the essence of QuickSort."', 
                   "Dean Galdiano The Coach of The Dream Team", 
                   "The Paul Method", 
                   "DIK A 💪 DIK A 💪 DIK A 💪"]
        self.app.quote_var = ctk.StringVar(value=random.choice(choices))
        ctk.CTkLabel(quote_frame, textvariable=self.app.quote_var, font=FONT_HEADER,
...
class RightPanel:
    ...
    def log(self):
        ...
        self.app.log_box = ctk.CTkTextbox(log_frame, fg_color=BG_INPUT, text_color=TEXT_PRIMARY,
        ...
        clear_btn = ctk.CTkButton(log_frame, text="Clear log", height=26, font=FONT_SMALL, 
                                  fg_color=BG_INPUT, hover_color=BG_PANEL, text_color=TEXT_SECONDARY, 
    ...
    def stats(self):
        ...
        stats = [
            ("Comparisons", self.app.comparison_var, COLOR_COMPARE),
            ("Swaps", self.app.swap_var, COLOR_SWAP),
            ("Recursion depth", self.app.depth_var, ACCENT_BRIGHT),
            ("Elapsed time", self.app.time_var, TEXT_SECONDARY),
        ]
    ...
    def subarray(self):
        ...
        for label, var, color in [
            ("Left index",  self.app.left_idx_var,  ACCENT_BRIGHT),
            ("Right index", self.app.right_idx_var, ACCENT_BRIGHT),
            ("Subarray",    self.app.subarray_var,  TEXT_PRIMARY),
        ]:
